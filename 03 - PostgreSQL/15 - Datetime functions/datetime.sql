...
select
	to_date('20 January 2026','DD, Month YYYY')
...
select 
    current_timestamp,
    to_char('2020-05-01 10:00:00'::timestamp, 'yyyy month dd');
...
select 
    ...
    to_char('2020-01-01 10:00:00'::timestamptz, 'yyyy month dd'),
    to_char('2020-01-01 10:00:00'::timestamptz, 'yyyy month dd hh:mi:ss tz'),
    to_char('2020-01-01 11:30:45'::timestamptz, 'fmmonth dd yyyy hh:mi:ss tz');
...
select 
    ...
    make_interval(2020, 03, 3, 5, 5, 1, 1),
...
select make_interval(YEARS => 5);


--make timestamp
select make_timestamptz(2000, 2, 2, 10, 00, 30,'ist');
SHOW timezone;
...
select extract('decade' from interval '500 years 11 months 24 days');
...
select (date '2000-01-01', date '2000-04-30') overlaps (date '2000-05-01', date '2000-12-31');
...
select age(current_date,'2004-11-24');

select age(timestamp '2004-08-20');

select age(current_date , timestamp '2004-11-24');

select age(timestamp '2004-11-24', current_date);
...
insert into time_update (update_on_date, add_time, entry)
values ('-infinity','allballs','-infinity');
...
values ('2020-11-24 12:45:00 us/alaska', '11:30:00+6');
...
select 
    date_part('week', timestamp '2017-01-25'),
    date_part('dow', timestamp '2017-05-04'),
    date_part('doy', timestamp '2017-12-20'),
